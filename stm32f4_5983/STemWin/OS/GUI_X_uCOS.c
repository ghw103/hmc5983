...
{
	OS_ERR      err;
  OSTimeDly(period, OS_OPT_TIME_PERIODIC, &err);
}
...
int    r;
	...
	CheckInit();
	if (KeyPressed == 0) {   
		OSSemPend((OS_SEM *)EventMbox,//请求信号量
		(OS_TICK )0,
		(OS_OPT  )OS_OPT_PEND_BLOCKING,
		(CPU_TS  )0,
		(OS_ERR *)&err);
